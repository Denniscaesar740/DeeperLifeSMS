...
branchesRouter.get('/:id', authenticateToken, async (req, res) => {
...
branchesRouter.post('/', authenticateToken, authorizeRoles('SUPER_ADMIN'), async (req, res) => {
...
branchesRouter.put('/:id', authenticateToken, authorizeRoles('SUPER_ADMIN', 'BRANCH_ADMIN'), async (req, res) => {
    const branchId = String(req.params.id);
    try {
        const updated = await BranchModel.findOneAndUpdate({ $or: [{ _id: branchId.match(/^[0-9a-fA-F]{24}$/) ? branchId : null }, { code: branchId }] }, req.body, { returnDocument: 'after' }).lean();
        if (!updated) {
            return res.status(404).json({ error: 'NOT_FOUND', message: 'Branch not found.' });
        }
...
branchesRouter.patch('/:id/status', authenticateToken, authorizeRoles('SUPER_ADMIN'), async (req, res) => {
    const branchId = String(req.params.id);
    const { status } = req.body;
    try {
        const updated = await BranchModel.findOneAndUpdate({ $or: [{ _id: branchId.match(/^[0-9a-fA-F]{24}$/) ? branchId : null }, { code: branchId }] }, { status }, { returnDocument: 'after' }).lean();
        if (!updated) {
            return res.status(404).json({ error: 'NOT_FOUND', message: 'Branch not found.' });
        }
