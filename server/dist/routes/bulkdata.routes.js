...
bulkdataRouter.post('/validate', authenticateToken, authorizeRoles('SUPER_ADMIN', 'BRANCH_ADMIN', 'ACCOUNTANT'), (req, res) => {
    ...
    records.forEach((row, index) => {
        ...
        if (targetEntity === 'STUDENTS') {
            if (!row.fullName)
                errors.push({ row: rowNo, field: 'fullName', message: 'Full name is required' });
            if (!row.level)
                errors.push({ row: rowNo, field: 'level', message: 'Grade level is required' });
            if (!row.guardianPhone)
                errors.push({ row: rowNo, field: 'guardianPhone', message: 'Guardian phone is required' });
        }
        else if (targetEntity === 'STAFF') {
            if (!row.fullName)
                errors.push({ row: rowNo, field: 'fullName', message: 'Full name is required' });
            if (!row.email)
                errors.push({ row: rowNo, field: 'email', message: 'Email address is required' });
        }
...
bulkdataRouter.post('/import', authenticateToken, authorizeRoles('SUPER_ADMIN'), async (req, res) => {
    const { targetEntity, records } = req.body;
    if (!targetEntity || !Array.isArray(records)) {
        return res.status(400).json({ error: 'BAD_REQUEST', message: 'targetEntity and records array are required.' });
    }
