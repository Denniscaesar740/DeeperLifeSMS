...
paymentsRouter.post('/initiate', authenticateToken, async (req, res) => {
...
paymentsRouter.post('/webhook', async (req, res) => {
    const signatureHeader = req.headers['x-hubtel-signature'] || req.headers['x-paystack-signature'];
    ...
    try {
        let txn = await PaymentTransactionModel.findOne({ referenceNo });
        if (!txn) {
...
paymentsRouter.get('/status/:referenceNo', authenticateToken, async (req, res) => {
    const referenceNo = String(req.params.referenceNo);
    try {
        const txn = await PaymentTransactionModel.findOne({ referenceNo }).lean();
        if (!txn) {
