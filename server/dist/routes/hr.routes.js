...
function calculateGhanaPayroll(basicSalaryGHS, allowancesGHS = 0) {
    ...
    if (tempIncome > 490) {
        ...
        if (tempIncome > 130) {
            ...
            payeTax += tempIncome * 0.10;
            tempIncome = 0;
        }
        if (tempIncome > 3166.67) {
            payeTax += 3166.67 * 0.175;
            tempIncome -= 3166.67;
        }
        else {
            payeTax += tempIncome * 0.175;
            tempIncome = 0;
        }
        if (tempIncome > 0) {
            payeTax += tempIncome * 0.25;
        }
    }
...
hrRouter.get('/staff', authenticateToken, async (_req, res) => {
    try {
...
hrRouter.post('/staff', authenticateToken, authorizeRoles('SUPER_ADMIN', 'BRANCH_ADMIN', 'ACCOUNTANT'), async (req, res) => {
    const { fullName, gender, phone, email, jobTitle, department, branchId, branchName, employmentType, salaryGHS, salary, classesAssigned, subjectsAssigned, status } = req.body;
    if (!fullName || !email) {
...
hrRouter.put('/staff/:id', authenticateToken, authorizeRoles('SUPER_ADMIN', 'BRANCH_ADMIN', 'ACCOUNTANT'), async (req, res) => {
    const { id } = req.params;
    try {
...
hrRouter.delete('/staff/:id', authenticateToken, authorizeRoles('SUPER_ADMIN', 'BRANCH_ADMIN'), async (req, res) => {
    const { id } = req.params;
    try {
...
hrRouter.post('/payroll/calculate', authenticateToken, authorizeRoles('SUPER_ADMIN', 'ACCOUNTANT'), (req, res) => {
...
hrRouter.post('/payroll/run', authenticateToken, authorizeRoles('SUPER_ADMIN', 'ACCOUNTANT'), async (req, res) => {
