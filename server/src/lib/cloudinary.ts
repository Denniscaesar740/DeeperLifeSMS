...
export interface CloudinaryUploadResult {
    ...
    url: string;
    publicId?: string;
    provider: 'cloudinary' | 'fallback_data_url';
    format?: string;
    bytes?: number;
    message: string;
}
 ...
 */
export async function uploadAvatarToCloudinary(fileData: string): Promise<CloudinaryUploadResult> {
    if (isCloudinaryConfigured()) {
        try {
            const res = await cloudinary.uploader.upload(fileData, {
                folder: 'dls_avatars',
 ...
 */
export async function uploadPassportToCloudinary(fileData: string): Promise<CloudinaryUploadResult> {
    if (isCloudinaryConfigured()) {
        try {
            const res = await cloudinary.uploader.upload(fileData, {
