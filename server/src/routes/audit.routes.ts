...
auditRouter.post('/logs', authenticateToken, async (req: Request, res: Response) => {
    const { action, module, details } = req.body;
    const userEmail = (req as any).user?.email || 'admin@dlschools.edu.gh';
...
auditRouter.get('/backup', authenticateToken, authorizeRoles('SUPER_ADMIN'), async (_req: Request, res: Response) => {
    try {
